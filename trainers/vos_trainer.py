...
class VOSTrainer(object):
    ...
    def train(self, max_epochs):
        for epoch in range(self._epoch, max_epochs + 1):
            ...
            if self._epoch % self._checkpoint_interval == 0:
                ...
                [
                    self._stats["val_vos mIoU"].history[-1] > i
    ...
    def cycle_dataset(self, mode):
        ...
        for i, vos_data in enumerate(loader_vos):
            ...
            if len(vos_out["logsegs"]) != 1:
                ...
                raise NotImplementedError(
                    "Model seems to track multiple targets during training, ids {}".format(
                        vos_out["logsegs"].keys()
                    )
                )
            ...
            loss["vos"] = self._objective_vos(
                vos_out["logsegs"][1].view(B * N, K, H, W),
                vos_segannos.view(B * N, H, W),
            )
            if vos_out.get("coarse_logsegs") is not None:
                loss["vos_aux1"] = self._objective_vos(
                    vos_out["coarse_logsegs"][1].view(B * N, K, H, W),
                    vos_segannos.view(B * N, H, W),
                )
            ...
            vos_iou = utils.get_intersection_over_union(
                ...
                vos_segannos.view(B * N, H, W).detach(),
            ).view(
                B, N, K
            )  # Care only about channel 1 (target)
            tmp = vos_iou[:, :, 1].mean(dim=1)
            if tmp.min() < vos_miou_extremes[0][0]:
                vos_miou_extremes[0] = (tmp.min(), i * B + tmp.argmin())
            if tmp.max() > vos_miou_extremes[1][0]:
                vos_miou_extremes[1] = (tmp.max(), i * B + tmp.argmax())
                ...
                if self._stats.get(mode + "_vos loss") is not None:
                    print(
                        "[{}: {}, {:5d}] Loss: {:.5f}".format(
                            ...
                            self._epoch,
                            i + 1,
                            self._stats[mode + "_vos loss"].avg,
                        )
                    )
        ...
        print(
            "[{}: {}] Loss vos: {:.5f}".format(
                mode, self._epoch, self._stats[mode + "_vos loss"].avg
            )
        )
        print(
            "[{}: {}] mIoU vos: {:.5f}".format(
                mode, self._epoch, self._stats[mode + "_vos mIoU"].avg
            )
        )
        print(
            "Worst mIoU this batch was {:.3f} (idx {}), and best {:.3f} (idx {})".format(
                vos_miou_extremes[0][0],
    ...
    def save_checkpoint(self, alternative_name=None):
        ...
        if alternative_name is not None:
            file_path = "{}/{}_{}.pth.tar".format(
                self._workspace_dir, self._save_name, alternative_name
            )
        elif self._save_name is None:
            file_path = "{}/{}_ep{:04d}.pth.tar".format(
                self._workspace_dir, net_type, self._epoch
            )
        else:
            file_path = "{}/{}_ep{:04d}.pth.tar".format(
                self._workspace_dir, self._save_name, self._epoch
            )
        ...
        if checkpoint is None:
            ...
            if self._save_name is None:
                checkpoint_list = sorted(
                    glob.glob("{}/{}_ep*.pth.tar".format(self._workspace_dir, net_type))
                )
            else:
                checkpoint_list = sorted(
                    glob.glob(
                        "{}/{}_ep*.pth.tar".format(self._workspace_dir, self._save_name)
                    )
                )
            if checkpoint_list:
            ...
            checkpoint_path = "{}/{}_ep{:04d}.pth.tar".format(
                self._workspace_dir, net_type, checkpoint
            )
