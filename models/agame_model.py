...
def softmax_aggregate(predicted_seg, object_ids):
    ...
    """
    bg_seg, _ = torch.stack(
        [seg[:, 0, :, :] for seg in predicted_seg.values()], dim=1
    ).min(dim=1)
    bg_seg = torch.stack([1 - bg_seg, bg_seg], dim=1)
    logits = {
        n: seg[:, 1:, :, :].clamp(1e-7, 1 - 1e-7)
        / seg[:, 0, :, :].clamp(1e-7, 1 - 1e-7)
    ...
    aggregated = torch.cat(
        [elem for lst in zip(aggregated_inv_lst, aggregated_lst) for elem in lst],
        dim=-3,
    )
    ...
    return (
        final_seg,
        {
            obj_idx: aggregated[:, 2 * (idx + 1) : 2 * (idx + 2), :, :]
            for idx, obj_idx in enumerate(object_ids)
        },
    )
...
def apply_padding(x, y, padding):
    ...
    y = [
        F.pad(label.float(), padding, mode="reflect").long()
...
def unpad(tensor, padding):
    if isinstance(tensor, (dict, OrderedDict)):
        ...
        tensor = tensor[
            :, :, :, padding[2] : height - padding[3], padding[0] : width - padding[1]
        ]
        return tensor
...
class DilationpyramidRelu(nn.Module):
    def __init__(
        self, nchannels_in, nchannels_out, kernel_size, stride, paddings, dilations
    ):
        super().__init__()
...
class GaussiansAgame(nn.Module):
    def __init__(
        self, nchannels_in, nchannels_lda, lr, cov_reg_init, residual=True, covest=True
    ):
        super().__init__()
        self.lr = lr
        ...
        self.cov_reg = nn.Parameter(
            cov_reg_init * torch.ones(4, 1, nchannels_lda, 1, 1)
        )
    ...
    def get_init_state(self, ref_feats, ref_seg):
        ...
        means = [
            F.adaptive_avg_pool2d(conv_ref_feats * ref_seg[:, i : i + 1, :, :], 1)
        ...
        if self.covest:
            covariances = [
                F.softplus(self.cov_reg[i])
                + F.adaptive_avg_pool2d(
                    distances[i] * distances[i] * ref_seg[:, i : i + 1, :, :], 1
                )
                / (1 / N + F.adaptive_avg_pool2d(ref_seg[:, i : i + 1, :, :], 1))
                for i in range(K1)
            ]
        ...
        if self.residual:
            test_classification = F.softmax(
                self.forward(ref_feats, {"appmod": (means, covariances)})[0], dim=-3
            )
            residual = F.relu(
                test_classification[:, :, :, :] - ref_seg[:, :, :, :]
            ).split(1, dim=1)
            means += [
                (
                    F.adaptive_avg_pool2d(conv_ref_feats * residual[idx], 1)
            ...
            if self.covest:
                covariances += [
                    F.softplus(self.cov_reg[i + 2])
                    + F.adaptive_avg_pool2d(
                        distances[i + 2] * distances[i + 2] * residual[i], 1
                    )
    ...
    def update(self, feats, seg, state):
        ...
        covariances = [
            (1 - self.lr) * o + self.lr * n
            for o, n in zip(old_covariances, new_covariances)
        ]
        return means, covariances
    ...
    def forward(self, feats, state):
        ...
        scores = [
            ...
            / 2
            * (
                torch.log(covariances[i])
                + (distances[i] * distances[i]) / covariances[i]
...
class GaussiansAgameHack(nn.Module):
    def __init__(
        ...
        if logprob_trainable_bias is not None:
            self.logprob_trainable_bias = nn.Parameter(
                logprob_trainable_bias * torch.ones(4)
            )
        else:
    ...
    def forward(self, feats, state):
        ...
        if self.logprob_trainable_bias is not None:
            scores = [
                scores[i] + self.logprob_trainable_bias[i] for i in range(nclasses)
            ]
        scores_tensor = torch.cat(scores, dim=-3)
        return scores_tensor, (means, covariances)
...
class RGMPLike(nn.Module):
    ...
    def forward(self, input_feats, state):
        ...
        h = torch.cat(
            [
                input_feats["s16"],
                dynmod_state["init_feats"],
                dynmod_state["prev_seg"],
                dynmod_state["init_seg"],
            ],
            dim=-3,
        )
        h = self.layers(h)
...
class UpsampleAgame(nn.Module):
    ...
    def forward(self, feats, fused_feats, state):
        ...
        h = torch.cat(
            [
                self.project_s8(feats["s8"]),
                F.upsample(h, scale_factor=2, mode="bilinear"),
            ],
            dim=-3,
        )
        h = self.blend_s8(h)
...
SUBMODS = {
    "DilationpyramidRelu": DilationpyramidRelu,
    "ConvRelu": ConvRelu,
    "Conv": Conv,
...
class AGAME(nn.Module):
    def __init__(
        ...
        backbone=(
            "resnet101s16",
            (True, ("layer4",), ("layer4",), ("layer2",), ("layer1",)),
        ),
        appearance=("GaussiansAgame", (2048, 512, 0.1, -2)),
        dynamic=(
            "RGMPLike",
            (
                [
                    ("ConvRelu", (2 * 2048 + 2 * 2, 512, 1, 1, 0)),
        ...
        update_with_fine_scores=False,
        update_with_softmax_aggregation=False,
        process_first_frame=True,
        output_logsegs=True,
        output_coarse_logsegs=True,
        output_segs=True,
    ):
        super().__init__()
    ...
    def forward(self, x, given_labels=None, state=None):
        ...
        video_frames = [
            elem.view(batchsize, nchannels, height, width) for elem in x.split(1, dim=1)
        ]
        ...
        if state is None:
            ...
            assert (
                init_label is not None
            ), "Model needs either a state, or info that permits its initialization"

            object_ids = init_label.unique().tolist()
            if 0 in object_ids:
                object_ids.remove(0)  # Background is not an object
            assert (
                len(object_ids) > 0
            ), "There are no objects given to track in the first frame"

            state = {}
            for obj_idx in object_ids:
                given_seg = F.avg_pool2d(
                    torch.cat(
                        [init_label != obj_idx, init_label == obj_idx], dim=-3
                    ).float(),
                    16,
                )
                state[obj_idx] = self.trackseg.get_init_state(
                    video_frames[0], given_seg
                )
        else:  # use previous state, update labels if needed
            object_ids = list(state.keys())
            init_label = (
                given_labels[0]
                if isinstance(given_labels, (tuple, list))
                else given_labels
            )
                ...
                for obj_idx in new_object_ids:
                    ...
                    if state.get(obj_idx) is None:
                        ...
                        raise NotImplementedError(
                            "Received a given (ground-truth) segmentation for an object idx that is already initialized. This could happen in the future, but should not happen with standard VOS datasets. Existing ids are {} and new ids are {}".format(
                                state.keys(), new_object_ids
                            )
                        )
        ...
        if (
            self.process_first_frame or given_labels[0] is None
        ):  # If no input_seg, process
        ...
        for i in frames_to_process:
            ...
            if (
                isinstance(given_labels, (list, tuple))
                and given_labels[i] is not None
                and i != 0
            ):
                ...
                for obj_idx in new_object_ids:
                    given_label_as_segmap_lst = [
                        given_labels[i] != obj_idx,
                        given_labels[i] == obj_idx,
                    ]
                    given_seg = F.avg_pool2d(
                        torch.cat(given_label_as_segmap_lst, dim=-3).float(), 16
                    )
                    if state.get(obj_idx) is None:
                        state[obj_idx] = self.trackseg.get_init_state(
                            video_frames[i], given_seg
                        )
            ...
            segscore = {}
            for k in object_ids:
                state[k], coarse_segscore[k], segscore[k] = self.trackseg(
                    feats, state[k]
                )

            # Merge scores for different objects (if necessary)
            predicted_seg = {
                k: F.softmax(segscore[k], dim=-3) for k in object_ids
            }  # {objidx: B x 2 x H x W}
            if self.output_segs or self.update_with_softmax_aggregation:
                assert len(predicted_seg.values()) > 0, "i: {}, objids: {}".format(
                    i, object_ids
                )
                output_seg, aggregated_seg = softmax_aggregate(
                    predicted_seg, object_ids
                )

            if self.update_with_softmax_aggregation:
                assert (
                    not self.training
                ), "Cannot update state with softmax-aggregated scores during training (yet)"
                update_seg = {
                    n: F.avg_pool2d(aggregated_seg[n], 16) for n in object_ids
                }
            elif self.update_with_fine_scores:
                assert (
                    not self.training
                ), "Cannot update state with fine scores during training, long recurrency..."
                update_seg = {k: F.avg_pool2d(predicted_seg[k], 16) for k in object_ids}
            else:
                update_seg = {
                    k: F.softmax(coarse_segscore[k], dim=-3) for k in object_ids
                }
            ...
            for k in object_ids:
                ...
                if self.output_coarse_logsegs:
                    coarse_logseg_lsts[k].append(
                        F.interpolate(
                            F.log_softmax(coarse_segscore[k], dim=-3),
                            scale_factor=16,
                            mode="bilinear",
                        )
                    )
        ...
        if self.output_logsegs:
            output["logsegs"] = {
                k: torch.stack(logseg_lsts[k], dim=1) for k in object_ids
            }
        if self.output_coarse_logsegs:
            output["coarse_logsegs"] = {
                k: torch.stack(coarse_logseg_lsts[k], dim=1) for k in object_ids
            }
        if self.output_segs:
        ...
        if tuple(required_padding) != (0, 0, 0, 0):
            ...
            if self.output_coarse_logsegs:
                output["coarse_logsegs"] = unpad(
                    output["coarse_logsegs"], required_padding
                )
            if self.output_segs:
