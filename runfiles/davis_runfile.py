...
def train_alpha(model):
    ...
    def label_read(path):
        if os.path.exists(path):
            ...
            label = torch.LongTensor(1, *size).fill_(
                255
            )  # Put label that will be ignored
        return label
    ...
    sampler = torch.utils.data.WeightedRandomSampler(
        len(train_set) * [1,], 2600, replacement=True
    )
    train_loader = DataLoader(
        train_set, sampler=sampler, batch_size=batch_size, num_workers=11
    )
    val_loader = DataLoader(
        val_set, shuffle=False, batch_size=batch_size, num_workers=11
    )
    print(
        "Sets initiated with {} (train) and {} (val) samples.".format(
    ...
    lr_sched = torch.optim.lr_scheduler.ExponentialLR(optimizer, 0.95)
...
def train_beta(model):
    ...
    sampler = torch.utils.data.WeightedRandomSampler(
        len(train_set) * [1,], 118, replacement=True
    )
    train_loader = DataLoader(
        train_set, batch_size=batch_size, sampler=sampler, num_workers=11
    )
    val_loader = DataLoader(
        val_set, shuffle=False, batch_size=batch_size, num_workers=11
    )
...
def test_model(
    model, debug_sequences_dict=None, save_predictions=False, prediction_path=None
):
    nframes = 128
    ...
    datasets = {
        ...
        "DAVIS16_val": DAVIS17V2(
            config["davis17_path"], "2016", "val", image_read, label_read, None, nframes
        ),
        "DAVIS17_val": DAVIS17V2(
            config["davis17_path"], "2017", "val", image_read, label_read, None, nframes
        ),
        "YTVOS_jval": YTVOSV2(
            config["ytvos_path"],
            "train",
    ...
    for key, dataset in datasets.items():
        if key == "YTVOS_val":
            evaluator = evaluation.VOSEvaluator(
                dataset, "cuda", "all", True, False, debug_sequences_dict.get(key)
            )
        elif key == "DAVIS17_val":
            evaluator = evaluation.VOSEvaluator(
                dataset, "cuda", "all", True, True, debug_sequences_dict.get(key)
            )
        else:
            evaluator = evaluation.VOSEvaluator(
                dataset, "cuda", "all", False, True, debug_sequences_dict.get(key)
            )
        result_fpath = os.path.join(
            config["output_path"], os.path.splitext(os.path.basename(__file__))[0]
        )
...
def main():
    print(
        "Started script: {}, with pytorch {}".format(
            os.path.basename(__file__), torch.__version__
        )
    )
    ...
    print(
        "Network model {} loaded, (size: {})".format(
            model.__class__.__name__, utils.get_model_size_str(model)
        )
    )
    ...
    if options.test is not None:
        ...
        model.load_state_dict(
            torch.load(checkpoint_basename + "_beta_best.pth.tar")["net"]
        )
        test_model(
            model,
            debug_sequences_dict=debug_sequences_dict,
