...
def train_alpha(model):
    ...
    train_loader = DataLoader(
        train_set, shuffle=True, batch_size=batch_size, num_workers=11
    )
...
def train_beta(model):
    ...
    sampler = torch.utils.data.WeightedRandomSampler(
        len(train_set.datasets[0]) * [1 / len(train_set.datasets[0])]
        + len(train_set.datasets[1]) * [1 / len(train_set.datasets[1])],
        2 * len(train_set.datasets[0]),
        replacement=False,
    )
    train_loader = DataLoader(
