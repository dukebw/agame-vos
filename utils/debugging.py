def get_tensor_statistics_str(tensor, name="", formatting="standard"):
    ...
    if isinstance(tensor, (torch.FloatTensor, torch.cuda.FloatTensor)):
        if formatting == "standard":
            string = "elem in [{:6.3f}, {:6.3f}]    mean: {:6.3f}    std: {:6.3f}    size: {}".format(
                tensor.min().item(),
                tensor.max().item(),
                tensor.mean().item(),
                tensor.std().item(),
            ...
            string = "[{:6.3f}, {:6.3f}]  mu: {:6.3f}  std: {:6.3f}  {!s:17} {: 6.1f}MB".format(
                tensor.min().item(),
                tensor.max().item(),
                tensor.mean().item(),
                tensor.std().item(),
                tuple(tensor.size()),
                4e-6 * prod(tensor.size()),
            )
    elif isinstance(
        tensor,
        (
        ...
        string = "elem in [{:6.3f}, {:6.3f}]    size: {}    HIST BELOW:\n{}".format(
            ...
            tuple(tensor.size()),
            torch.stack(
                [torch.arange(0, tensor.max() + 1), tensor.view(-1).bincount()], dim=0
            ),
        )
    else:
        raise NotImplementedError(
            "A type of tensor not yet supported was input. Expected torch.FloatTensor or torch.LongTensor, got: {}".format(
                tensor.type()
            )
        )
    string = string + "    " + name
