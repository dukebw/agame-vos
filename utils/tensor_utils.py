...
def get_intersection_over_union(predictions, gt):
    ...
    assert gt.size(0) == nsamples, "gt size: {},  predictions size: {}".format(
        gt.size(), predictions.size()
    )
    assert gt.size(1) == height, "gt size: {},  predictions size: {}".format(
        gt.size(), predictions.size()
    )
    assert gt.size(2) == width, "gt size: {},  predictions size: {}".format(
        gt.size(), predictions.size()
    )
    prediction_max, prediction_argmax = predictions.max(-3)
    prediction_argmax = prediction_argmax.long()
    classes = gt.new_tensor([c for c in range(nclasses)]).view(
        1, nclasses, 1, 1
    )  # [1,K,1,1]
    pred_bin = (
        prediction_argmax.view(nsamples, 1, height, width) == classes
    )  # [N,K,H,W]
