...
class VOSEvaluator(object):
        ...
        if "seg" in save_predicted_segmentations:
            ...
            if dataset.__class__.__name__ == "DAVIS17V2":
                ...
                raise NotImplementedError(
                    "Requested to save predicted segmentations with a dataset where this functionality is not supported. Dataset was {}".format(
    ...
    def read_video_part(self, video_part):
        ...
        given_segannos = [
            seganno.to(self._device) if seganno is not None else None
            for seganno in video_part["given_segannos"]
        ]
        segannos = (
            video_part["segannos"].to(self._device)
            if video_part.get("segannos") is not None
            else None
        )
    ...
    def get_seg_measure(self, tracker_out, segannos, frame_idx0, seqname, obj_idx):
        ...
        if N - frame_idx0 < 3:
            print(
                "Object {} in sequence {} was tracked for only {} frames, should be at least 3 frames, setting iou to 1.".format(
                    obj_idx, seqname, N - frame_idx0
                )
            )
            measure = 1.0
        else:
            obj_segannos = (
                (segannos[:, frame_idx0:, :, :] == obj_idx).long().view(N, H * W)
            )
            obj_preds = (
                (tracker_out["segs"][:, frame_idx0:, :, :, :] == obj_idx)
                .long()
                .view(N, H * W)
            )
            intersection = torch.min(obj_segannos, obj_preds).sum(-1)
            union = torch.max(obj_segannos, obj_preds).sum(-1)
            measure = torch.clamp(intersection.float(), min=1e-10) / torch.clamp(
                union.float(), min=1e-10
            )  # size N
        return measure
    ...
    def add_measures_for_chunk(self, measure_lsts, tracker_out, segannos, seqname):
        for obj_idx, first_frame_idx in tracker_out["object_visibility"].items():
            ...
            if "iou_seg" in self._calculate_measures:
                measure = self.get_seg_measure(
                    tracker_out, segannos, first_frame_idx, seqname, obj_idx
                )
    ...
    def evaluate_video(self, model, seqname, video_parts, output_path):
        ...
        for video_part in video_parts:
            ...
            try:
                tracker_out, tracker_state = model(
                    images, given_segannos, tracker_state
                )
            except:
                print(
                    "Crash in model for seq {}, given segannos: {}".format(
                        seqname,
                        [
                            elem.size() if elem is not None else None
                            for elem in given_segannos
                        ],
                    )
                )
                raise
            ...
            if "seg" in self._save_predicted_segmentations:
                ...
                for idx in range(len(fnames)):
                    ...
                    data = (
                        (
                            tracker_out["segs"][0, idx, 0, :, :].cpu().byte().numpy(),
                            fpath,
                        ),
                        self._sdm,
                    )
            ...
            if self._calculate_measures:
                measure_lsts = self.add_measures_for_chunk(
                    measure_lsts, tracker_out, segannos, seqname
                )
        ...
        if self._debug_sequences:
            raise NotImplementedError(
                "Joakim probably played ping-pong instead of implementing this."
            )
    ...
    def evaluate(self, model, output_path):
        ...
        with torch.no_grad():
            for seqname, video_parts in self._dataset.get_video_generator():
                if (
                    self._sequences_to_eval is not "all"
                    and seqname not in self._sequences_to_eval
                ):
                    continue
                t1 = time.time()
                ...
                if (
                    ...
                    and len(os.listdir(savepath)) > 0
                    and self._skip_existing
                ):
                ...
                try:
                    measures = self.evaluate_video(
                        model, seqname, video_parts, output_path
                    )
                except:
                    print(seqname)
                    raise
        ...
        if self._calculate_measures:
            performance_measures["iou"]["mean"] = (lambda lst: sum(lst) / len(lst))(
                [
                    seq_measure["mean"]
                    for seq_measure in performance_measures["iou"]["perseq"].values()
                ]
            )
            ...
            print(
                "Storing result in {} with a mean iou of {:.3f}".format(
