        ...
        transform = tv.transforms.Compose(
            [
                ...
                tv.transforms.Normalize(
                    mean=dataset_utils.IMAGENET_MEAN, std=dataset_utils.IMAGENET_STD
                ),
            ]
        )
...
class YTVOSV2(torch.utils.data.Dataset):
    def __init__(
        ...
        image_set,
        impath="JPEGImages",
        image_read=get_default_image_read(),
        anno_read=get_default_anno_read(),
        joint_transform=None,
        samplelen=4,
        obj_selection=get_sample_all(),
        min_num_obj=1,
        start_frame="random",
    ):
        ...
        assert (
            image_set in ("train", "train_joakim", "val_joakim") and split == "train"
    ...
    def _init_data(self):
        ...
        cache_path = os.path.join(
            framework_path,
            "cache",
            "ytvos_v2_{}_100px_threshold.json".format(self._split),
        )
        ...
        if os.path.exists(cache_path):
            ...
            seqnames = os.listdir(
                os.path.join(self._root_path, self._split, self._impath)
            )
        ...
        if self._split == "train":
            with open(
                os.path.join(self._root_path, "ImageSets", self._image_set + ".txt"),
                "r",
            ) as f:
            ...
            self._all_seqs = os.listdir(
                os.path.join(self._root_path, self._split, "Annotations")
            )
            print(
                "{} sequences found in the Annotations directory.".format(
                    len(self._all_seqs)
                )
            )
        ...
        print(
            "{} sequences remaining after filtering out sequences that are not in 720p.".format(
                len(self._viable_seqs)
            )
        )
    ...
    def get_anno_frame_names(self, seqname):
        return os.listdir(
            os.path.join(self._root_path, self._split, "Annotations", seqname)
        )

    def get_anno_frame_ids(self, seqname):
        return sorted(
            [
                self._frame_name_to_idx(fname)
                for fname in self.get_anno_frame_names(seqname)
            ]
        )

    def get_image_frame_names(self, seqname):
        return os.listdir(
            os.path.join(self._root_path, self._split, self._impath, seqname)
        )
    ...
    def get_image_frame_ids(self, seqname):
        return sorted(
            [
                self._frame_name_to_idx(fname)
                for fname in self.get_image_frame_names(seqname)
            ]
        )
    ...
    def _select_object_ids(self, labels):
        assert labels.min() > -1 and labels.max() < 256, "{}".format(
            utils.print_tensor_statistics(labels)
        )
        possible_obj_ids = (
            (labels[0].view(-1).bincount() > 10).nonzero().view(-1).tolist()
        )
    ...
    def __getitem__(self, idx):
        ...
        viable_starting_frame_ids = [
            idx
            for idx in self.get_nonempty_frame_ids(seqname)
        ...
        images = torch.stack(
            [self._image_read(self._full_image_path(seqname, idx)) for idx in frame_ids]
        )
        segannos = torch.stack(
            [self._anno_read(self._full_anno_path(seqname, idx)) for idx in frame_ids]
        )
        ...
        return {
            "images": images,
            "provides_seganno": provides_seganno,
            "given_seganno": given_seganno,
            "segannos": segannos,
        }
    ...
    def _get_snippet(self, seqname, frame_ids):
        ...
        if self._split == "valid":
            ...
            given_segannos = [
                self._anno_read(self._full_anno_path(seqname, idx)).unsqueeze(0)
                if idx in anno_frame_ids
                else None
            ...
            given_segannos = [
                self._anno_read(self._full_anno_path(seqname, idx)).unsqueeze(0)
                if idx == self.get_anno_frame_ids(seqname)[0]
                else None
                for idx in frame_ids
            ]
    ...
    def _get_video(self, seqname):
        ...
        partitioned_frame_ids = [
            seq_frame_ids[start_idx : start_idx + self._seqlen]
