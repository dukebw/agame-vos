...
class LabelToLongTensor(object):
    ...
    def __call__(self, pic):
        if isinstance(pic, np.ndarray):
            ...
            label = (
                torch.from_numpy(np.array(pic, np.uint8, copy=False))
                .long()
                .view(1, pic.size[1], pic.size[0])
            )
        else:
...
class JointRandomScale(object):
    def __call__(self, images, labels):
        ...
        scales = (
            (
                (1.0 + (torch.rand(1) < 0.5).float() * torch.rand(1) * 0.1)
                * torch.ones(L)
            )
            .cumprod(0)
            .tolist()
